...
**所有功能仅在 ThinkBook 16p G6 IAX、BIOS R2CN57WW 上进行过测试；功耗参数写入在 ThinkBook 16p G5 IRX（2024 款）上验证可用。不保证任何功能在其它机型或 BIOS 版本上可用或安全。功能检测只决定界面显示哪些项目，不代表兼容性或安全性已经得到验证。ThinkBook 16p G5 系列（2024 款）的风扇由固件自动控制，不支持手动调节。**
